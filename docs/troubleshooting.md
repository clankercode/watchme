...
| Herdr protocol error mentions unknown field `id` | Older WatchMe tried its bridge contract against native Herdr protocol 16 | Upgrade WatchMe; current builds negotiate native protocol 16 explicitly |
...
- **Herdr**: Herdr 0.7.4 protocol 16 is supported for exact pane identity, bounded reads, and atomic input. Unsupported protocol versions degrade to verified process supervision; unsafe sockets or identity contradictions fail closed.
